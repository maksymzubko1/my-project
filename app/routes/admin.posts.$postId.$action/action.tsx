...
export const action = async ({ params, request }: ActionFunctionArgs) => {
  ...
  if (
    !action ||
    !["soft_delete", "hide", "show", "delete", "restore"].includes(action)
  ) {
    return json(
  ...
  try {
    switch (action) {
      case "soft_delete":
        await changePostStatus({ id: params.postId, isDeleted: true });
        return json(
          { status: "success", message: "Post has been soft deleted" },
          { status: 200 },
        );
      case "delete":
      ...
      case "hide":
        ...
        return json(
          { status: "success", message: "Post display has been disabled" },
          { status: 200 },
        );
      case "show":
        await changePostStatus({ id: params.postId, status: "DEFAULT" });
        return json(
          { status: "success", message: "Post display has been enabled" },
          { status: 200 },
        );
      case "restore":
        await changePostStatus({ id: params.postId, isDeleted: false });
        return json(
          { status: "success", message: "Post has been restored" },
          { status: 200 },
        );
    }
  } catch (e) {
