...
const Modal = ({ open, onClose, children, title, actions, isLoading }: ModalProps) => {
  useEffect(() => {
    if (open) {
  ...
  return (
    <div tabIndex="-1" aria-hidden={!open}
      ...
      <div className="relative p-4 w-full max-w-2xl max-h-full">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div className="flex items-center p-4 md:p-5 gap-2 justify-end border-t border-gray-200 rounded-b dark:border-gray-600">
            ...
            {actions.map(action => (
              <Button loading={action.isLoading || isLoading} onClick={action.onAction} variant={action.buttonType}>
                {action.text}
              </Button>
            ))}
          </div>
