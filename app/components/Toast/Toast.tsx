...
const toastStyles = {
  ...
  warning: {
    style: 'text-orange-500 bg-orange-100 rounded-lg dark:bg-orange-700 dark:text-orange-200',
...
const ToastComponent = ({variant, message, id, onClick}: Toast) => {
  return (
    <div id={`toast-${variant}-${id}`}
         className={`flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800`}
         role="alert">
      <div
        className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 ${toastStyles[variant].style}`}>
        <svg className="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
             viewBox="0 0 20 20">
          <path
            d={toastStyles[variant].icon} />
        </svg>
        <span className="sr-only">{toastStyles[variant].name}</span>
      </div>
      <div className="ms-3 text-sm me-3 font-normal">{message}</div>
      <button type="button"
              className="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700"
              data-dismiss-target="#toast-success" aria-label="Close" onClick={onClick}>
